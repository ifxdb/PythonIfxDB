...
class IfxDbTestCase(unittest.TestCase):
  ...
  def run_test_042(self):
    ...
    #if ({}['EMPNO'] != None):
    #  result = ifx_db.exec_immediate(conn, "select photo_format, picture, length(picture) from emp_photo where photo_format='jpg' and empno='" + {}['EMPNO'] + "'")
    #  row = ifx_db.fetch_array(result)
    #  if row:
    ...
    #  continue
    #else:
    result = ifx_db.exec_immediate(conn, "select EMPNO, PHOTO_FORMAT from emp_photo where photo_format='jpg'")
