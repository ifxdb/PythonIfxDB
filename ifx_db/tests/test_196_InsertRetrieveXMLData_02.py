...
class IfxDbTestCase(unittest.TestCase):
  ...
  def run_test_196(self):
    ...
    if ((server.DBMS_NAME[0:3] != 'IDS') and (server.DBMS_NAME[0:2] != "AS")):
      try:
          rc = ifx_db.exec_immediate(conn, "DROP TABLE xml_test")
      except:
          pass
      rc = ifx_db.exec_immediate(conn, "CREATE TABLE xml_test (id INTEGER, data VARCHAR(50), xmlcol XML)")
      rc = ifx_db.exec_immediate(conn, "INSERT INTO xml_test (id, data, xmlcol) values (1, 'xml test 1', '<address><street>12485 S Pine St.</street><city>Olathe</city><state>KS</state><zip>66061</zip></address>')")

      sql =  "SELECT * FROM xml_test"
      stmt = ifx_db.prepare(conn, sql)
      ifx_db.execute(stmt)
      result = ifx_db.fetch_both(stmt)
      while( result ):
