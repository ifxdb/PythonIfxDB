...
class IfxDbTestCase(unittest.TestCase):
  ...
  def run_test_230(self):
    ...
    for i in range(0, ifx_db.num_fields(result) + 1):
      print str(i) + ":" + str(ifx_db.field_type(result,i))
    
    print "\n-----"
    
    for i in range(0, ifx_db.num_fields(result2)):
      print str(i) + ":" + str(ifx_db.field_type(result2,i))
      
    print "\n-----"

    for i in range(0, 3):
      print str(i) + ":" + str(ifx_db.field_type(result3,i))

    print "\n-----"
    
    print "region:%s" % str(ifx_db.field_type(result,'region'))
    print "5:%s" % str(ifx_db.field_type(result2,5))
