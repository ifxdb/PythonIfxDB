...
class IfxDbTestCase(unittest.TestCase):
    ...
    def run_test_execute_many(self):
        ...
        if conn:
            ...
            stmt_insert = ifx_db.prepare(conn, insert)
            ifx_db.execute_many(stmt_insert, params)
            #check the number of rows inserted
            row_count = ifx_db.num_rows(stmt_insert)
            print row_count
            
            # chaeck the inserted columns
            select = "SELECT * FROM TABMANY"
            stmt_select = ifx_db.exec_immediate(conn, select)
            cols = ifx_db.fetch_tuple( stmt_select )
            while( cols ):
